...
void XRWebGLSwapChain::ClearCurrentTexture() {
  ...
  if (!texture) {
    DVLOG(3) << __func__ << " No texture";
    return;
  }

  gpu::gles2::GLES2Interface* gl = context()->ContextGL();
  if (!gl) {
    DVLOG(3) << __func__ << " NO Gl";
    return;
  }
  ...
  if (attachment == GL_COLOR_ATTACHMENT0) {
    DVLOG(3) << __func__ << " Performing color clear";
    clear_bits |= GL_COLOR_BUFFER_BIT;
    gl->ColorMask(true, true, true, true);
    gl->ClearColor(0, 0, 0, 0);
  } else if (attachment == GL_DEPTH_ATTACHMENT) {
    DVLOG(3) << __func__ << " Performing depth clear";
    clear_bits |= GL_DEPTH_BUFFER_BIT;
    gl->DepthMask(true);
    gl->ClearDepthf(1.0f);
  } else if (attachment == GL_STENCIL_ATTACHMENT) {
    DVLOG(3) << __func__ << " Performing stencil clear";
    clear_bits |= GL_STENCIL_BUFFER_BIT;
    gl->StencilMaskSeparate(GL_FRONT, true);
    gl->ClearStencil(0);
  } else {
    DVLOG(3) << __func__ << " Unknown Attachment, not setting clear bits";
  }
  ...
  if (descriptor_.is_texture_array) {
    DVLOG(3) << __func__ << " Performing texture array clear";
    ...
    DVLOG(3) << __func__ << " Performing cube clear";
    ...
    DVLOG(3) << __func__ << " Performing default clear";
